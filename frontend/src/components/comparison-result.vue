...
import {DelComparisonResult, GetComparisonResult} from "../../wailsjs/go/main/App";
    ...
    if (res.ret === 1 && res.data.List !== null) {
      for (var i = 0; i < res.data.List.length; i++) {
        ...
        data.value.push({
          ...
          fileSizeA: item.SizeA,
          base64A: item.Base64A,

          pathB: item.PathB,
          fileNameB: item.NameB,
          fileSizeB: item.SizeB,
      ...
      >
        <template #bodyCell="{column, text, record}" >
          ...
          <template v-if="column.dataIndex === 'fileNameB'">
            <div class="file-info">
              <span class="file-name">{{ text }}</span><br/>
              <span  class="file-size">{{ record.fileSizeB }}</span>
            </div>
            <a-image class="file-img"
                :width="60"
          ...
          <template v-if="column.dataIndex === 'key'">
            <a-button @click="viewSourceFile()" class="del-a" type="link" :icon="h(FolderOpenOutlined)">
              <span>查看源文件</span>
            </a-button>
          </template>
